class SubscriptionManager:
    def get_active_package(request):
        return f"""
        ...
        WHERE t.username = '{request.session['username']}' AND t.end_date_time >= CURRENT_TIMESTAMP
    ...
    def get_transaction_history(request):
        ...
        FROM pacilflix.PAKET AS p
        JOIN pacilflix.TRANSACTION AS t ON p.nama = t.nama_paket
        WHERE t.username = '{request.session['username']}'
    ...
    def purchase_package(request, nama_paket, metode_pembayaran):
        ...
        VALUES ('{request.session['username']}', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP + INTERVAL '1 month', '{nama_paket}', '{metode_pembayaran}', CURRENT_TIMESTAMP);
        """